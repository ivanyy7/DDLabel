...
- **`client/`** — React (Vite): `src/bluetoothPrint.js` (Web Serial API, печать по Bluetooth SPP); **верхнее меню** — три вкладки (Главная, Справочник, Настройки); **Главная** — поле ввода фразы (многострочное, 450 px, resize), кнопка «Р» (Разобрать) справа от поля, кнопки «Голос», «на Печать», «Двойной/Одиночн», «Сброс»; переключатель режима этикетки (одиночный — зелёный); голос: «ок» — команда печати (задержка 450 мс), не пишется в поле; **пакетная печать** — «N штук» (2–50) в конце шаблона, поддержка слов (два, три, четыре…); **очистка поля** после успешной печати; **голос** — полный транскрипт из event.results, накопление между сессиями, слияние «штуки» с шаблоном, labelModeRef для актуального режима; **Справочник** — раздел «Справочник сроков», окно «Список продуктов» с прокруткой (max-height 75vh); **Настройки** — переключатель темы, превью этикетки, панель TSPL. Шрифт превью — **Teko** (Google Fonts). Прокси `/api`. Запуск: `npm run dev`. http://localhost:3000.
...
- **Планируется (Этап 2):** 8.3 Раздел «Печать» в Настройках → 8.4 Синхронизация справочника → 8.6 Офлайн. Формулировки 8.3, 8.4, 8.6 в PLAN.md уточнены: офлайн/онлайн с телефона, локальный справочник и синхронизация с Vercel, парсер на клиенте с fallback и сообщением при сбое сети.
