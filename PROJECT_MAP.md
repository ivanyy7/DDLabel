...
**Этап 1 (в работе):**
- **`server/`** — Node.js (Express): `index.js` (API печати POST /api/print, POST /api/test-print для калибровки, POST /api/parse, CRUD справочника, GET /api/printers, GET /api/tspl-fonts), **`tsplDriver.js`** (TSPL: buildTsplLabel — полная этикетка 30×20 мм со старыми захардкоженными параметрами; buildTsplTestDates — калибровочная печать с прямыми TSPL-параметрами: название + даты + время, font/sx/sy/x/y/density/speed; TSPL_FONTS — справочник встроенных шрифтов; CP866), labelBuilder.js (устаревший ESC/POS), phraseParser.js, shelfLife.js, shelfStorage.js. Печать: сырые команды TSPL на принтер по USB. Запуск: `npm start`. Порт 3001.
