...
- **`client/`** — React (Vite): **верхнее меню** — три вкладки (Главная, Справочник, Настройки); **Главная** — поле ввода фразы (многострочное, 450 px, resize), кнопка «Р» (Разобрать) справа от поля, кнопки «Голос», «на Печать», «Двойной/Одиночн», «Сброс»; переключатель режима этикетки (одиночный — зелёный); голос: «ок» — команда печати (задержка 450 мс), не пишется в поле; **пакетная печать** — «N штук» (2–50) в конце шаблона, поддержка слов (два, три, четыре…); **очистка поля** после успешной печати; **голос** — полный транскрипт из event.results, накопление между сессиями, слияние «штуки» с шаблоном, labelModeRef для актуального режима; **Справочник** — раздел «Справочник сроков», окно «Список продуктов» с прокруткой (max-height 75vh); **Настройки** — переключатель темы, превью этикетки, панель TSPL. Шрифт превью — **Teko** (Google Fonts). Прокси `/api`. Запуск: `npm run dev`. http://localhost:3000.
