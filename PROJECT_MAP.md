...
- **`client/`** — React (Vite): **верхнее меню** — три вкладки (Главная, Справочник, Настройки), выровненные по правому краю; **Главная** — поле ввода фразы (многострочное, resize за нижнюю границу), кнопки «Голос», «на Печать», «Разобрать», «Сброс»; **Справочник** — раздел «Справочник сроков» (добавление, список, редактирование); **Настройки** — переключатель темы Светлая/Тёмная (localStorage), блок «Превью макета этикетки» с **панелью TSPL-настроек** (font, sx, sy, x, y, DENSITY, SPEED, WIDTH_PRESETS, шаблоны в localStorage, кнопка «Тестовая печать»). Шрифт превью — **Teko** (Google Fonts). Прокси `/api` на сервер. Запуск: `npm run dev`. Интерфейс: http://localhost:3000 (port 3000, host: true).
